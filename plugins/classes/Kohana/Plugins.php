...
class Kohana_Plugins
{
    ...
    public function __construct($params = array())
    {
        ...
        {
            $this->set_plugin_dir($config->get("plugins_dir"));
        }
    ...
    public function find_plugins()
    {
        ...
        $directory = new DirectoryIterator(self::$plugins_dir);
        ...
        if ($directory->isDir())
        {
            foreach ($directory AS $plugin)
            {
                ...
                if ( $plugin->isDir() && !$plugin->isDot() )
                {
                    ...
                    $config = require_once self::$plugins_dir . $plugin->getFilename() . DIRECTORY_SEPARATOR . "config.php";
    ...
    public function include_plugins()
    {
        if ( self::$plugins_active AND !empty(self::$plugins_active) )
        {
            ...
            {
                include_once self::$plugins_dir . $name . DIRECTORY_SEPARATOR . "init.php";
